...
# procedure sort
sort:
	addi	$sp,$sp,-20	#	preservação de registradores armazenando seus conteudos na pilha
	sw		$ra,16($sp)	#	salva $ra em sp[4]
	sw		$s1,4($sp)	#	salva $s1 em sp[1]
	sw		$s0,0($sp)	#	salva $s0 em sp[0]
	sw		$s2,8($sp)	# salva $s2 em sp[2]
	sw		$s3,12($sp)	#	salva $s3 em sp[3]
	
	move	$s2,$a0			# salva o valor de $a0 (v) em $s3
	move 	$s3,$a1			# salva o valor de $a1 (n) em $s3
	...
	lw		$s3,12($sp)	#	salva $s3 em sp[3]
	lw		$s2,8($sp)	# salva $s2 em sp[2]
	lw		$s0,0($sp)	#	$s0 = sp[0]
	lw		$s1,4($sp)	#	$s1 = sp[1]
	lw		$ra,16($sp)	#	$ra = sp[2]
	addi	$sp,$sp,20	
